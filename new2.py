...
class PatientAgent(mesa.Agent):
    def __init__(self, unique_id, model, init_state=ALIVE):
        ...
        self.injury_level = random.randint(1, 10)
    ...
    def step(self):
        if self.TTL <= 0:
            ...
            self.TTL -= int(self.injury_level / 2)
...
class DoctorAgent(mesa.Agent):
    ...
    def step(self):
        ...
        if path_to_patient:
            self.model.grid.move_agent(self, path_to_patient[0])
    ...
    def locate_patient(self):
        ...
        alive_patients = [agent for agent in all_agents if agent.type == PATIENT and agent.state == ALIVE]
